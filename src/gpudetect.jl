# GPU backend discovery for test suites. Probes the system without loading any
# GPU package, installs the matching backend on demand into the (sandboxed) test
# env, and degrades gracefully to CPU. Replaces the copy-pasted `utils_gpu.jl`
# include-scripts across the ACEsuit packages.
